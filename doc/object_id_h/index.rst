...
:ecudoxygen:`ECU_USER_OBJECT_ID_BEGIN` is guaranteed to always be 0. IDs less than :ecudoxygen:`ECU_USER_OBJECT_ID_BEGIN` are reserved for internal use by ECU library. Therefore reserved IDs are always negative. User-defined IDs are always >= 0.

Example
-------------------------------------------------
The following example stores different node types in the same linked list (:ref:`dlist.h <dlist_h>`) and uses the object ID scheme to identify them:
